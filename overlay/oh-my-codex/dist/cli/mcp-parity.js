...
async function importWithAutoStartDisabled(envName, importer) {
...
function buildDescriptorHelp(descriptor) {
...
export function parseMcpCliArgs(args) {
...
async function executeDescriptorCommand(args, loadDescriptor) {
    ...
    if (!allowedTools.has(toolName)) {
        throw new Error(`Unknown ${descriptor.commandName} tool: ${parsed.toolName}\n${buildDescriptorHelp(descriptor)}`);
    }
...
async function runDescriptorCommand(args, loadDescriptor) {
    const parsed = parseMcpCliArgs(args);
...
async function loadStateDescriptor() {
    const { buildStateServerTools, handleStateToolCall } = await importWithAutoStartDisabled("OMX_STATE_SERVER_DISABLE_AUTO_START", async () => await import("../mcp/state-server.js"));
    return {
        commandName: "state",
...
async function loadMemoryDescriptor(commandName, prefix, title) {
    const { buildMemoryServerTools, handleMemoryToolCall } = await importWithAutoStartDisabled("OMX_MEMORY_SERVER_DISABLE_AUTO_START", async () => await import("../mcp/memory-server.js"));
    return {
        commandName,
        title,
...
async function loadGenericMemoryDescriptor() {
    const { buildMemoryServerTools, handleMemoryToolCall } = await importWithAutoStartDisabled("OMX_MEMORY_SERVER_DISABLE_AUTO_START", async () => await import("../mcp/memory-server.js"));
    return {
        commandName: "memory",
        title: "CLI parity surface for OMX backend-aware memory tools.",
        tools: buildMemoryServerTools()
            .filter((tool) => tool.name.startsWith("memory_backend_") || tool.name === "memory_routed_write")
            .map(({ name, description }) => ({ name, description })),
        aliases: {
            "list-backends": "memory_backend_list",
            search: "memory_backend_search",
            write: "memory_routed_write",
        },
        handle: handleMemoryToolCall,
    };
}
async function loadTraceDescriptor() {
    const { buildTraceServerTools, handleTraceToolCall } = await importWithAutoStartDisabled("OMX_TRACE_SERVER_DISABLE_AUTO_START", async () => await import("../mcp/trace-server.js"));
    return {
        commandName: "trace",
...
async function loadCodeIntelDescriptor() {
    const { buildCodeIntelServerTools, handleCodeIntelToolCall } = await importWithAutoStartDisabled("OMX_CODE_INTEL_SERVER_DISABLE_AUTO_START", async () => await import("../mcp/code-intel-server.js"));
    return {
...
async function loadWikiDescriptor() {
    const { buildWikiServerTools, handleWikiToolCall } = await importWithAutoStartDisabled("OMX_WIKI_SERVER_DISABLE_AUTO_START", async () => await import("../mcp/wiki-server.js"));
    return {
        commandName: "wiki",
...
export async function mcpParityCommand(commandName, args) {
    switch (commandName) {
        case "state":
            await runDescriptorCommand(args, loadStateDescriptor);
            return;
        case "notepad":
            await runDescriptorCommand(args, async () => await loadMemoryDescriptor("notepad", "notepad_", "CLI parity surface for OMX notepad MCP tools."));
            return;
        case "project-memory":
            await runDescriptorCommand(args, async () => await loadMemoryDescriptor("project-memory", "project_memory_", "CLI parity surface for OMX project-memory MCP tools."));
            return;
        case "memory":
            await runDescriptorCommand(args, loadGenericMemoryDescriptor);
            return;
        case "trace":
...
export async function executeMcpParityCommand(commandName, args) {
    switch (commandName) {
        case "state":
            return await executeDescriptorCommand(args, loadStateDescriptor);
        case "notepad":
            return await executeDescriptorCommand(args, async () => await loadMemoryDescriptor("notepad", "notepad_", "CLI parity surface for OMX notepad MCP tools."));
        case "project-memory":
            return await executeDescriptorCommand(args, async () => await loadMemoryDescriptor("project-memory", "project_memory_", "CLI parity surface for OMX project-memory MCP tools."));
        case "memory":
            return await executeDescriptorCommand(args, loadGenericMemoryDescriptor);
        case "trace":
...
//# sourceMappingURL=mcp-parity.js.map
