...
This repository is an **OMX-orchestrated Codex workspace for `.openclaw`**, not an OpenClaw-native orchestration surface.

Project-specific sources of truth:
- `workspace/AGENTS.md` for workspace operating rules and memory behavior
...
- Surface material assumptions and tradeoffs instead of silently choosing a path.
- Prefer the simplest sufficient implementation; avoid speculative flexibility or abstractions.
...
- For non-trivial work, define the success proof before editing, then verify against it after the change.
...
Supported workflow triggers include: `ralph`, `autopilot`, `ultrawork`, `ultraqa`, `cleanup`/`refactor`/`deslop`, `analyze`, `plan this`, `deep interview`, `ouroboros`, `ralplan`, `team`/`swarm`, `ecomode`, `cancel`, `tdd`, `fix build`, `multifix`, `code review`, `security review`, and `web-clone`.
...
| "multifix", "/multifix" | `$multifix` | Read `./.codex/skills/multifix/SKILL.md`, run the local bug-fix lane: investigate, choose the smallest safe fix, verify, and learn |
