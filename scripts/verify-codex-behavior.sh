...
write_result=$(./omx memory write --input "{\"classification\":\"repo_state\",\"problem\":\"verification test\",\"context\":\"behavior check\",\"solution\":\"test solution\",\"failure\":\"none\",\"confidence\":\"verified\",\"source\":\"verify-behavior\",\"tags\":[\"test\"],\"verified\":true,\"workingDirectory\":\"$WORKDIR\"}" --json 2>&1) || true
...
search_result=$(./omx memory search --input "{\"query\":\"verification test\",\"backend\":\"all\",\"workingDirectory\":\"$WORKDIR\"}" --json 2>&1) || true
...
backends_result=$(./omx memory list-backends --input "{\"workingDirectory\":\"$WORKDIR\"}" --json 2>&1) || true
